...
#SBATCH --output=paper_results.log
#SBATCH --mem=32G
#SBATCH -n 4
...
class Custom_GRU(nn.Module):
    ...
    def train_test_gru(self, train_inp, train_out, test_inp, test_act_out, test_out):
        predictions = []
...
for parameter_combo in all_parameter_combinations:
    ...
    if(config["attention"] == True):
        config["result_filename"] = "results/paper_final/W_Decay_{}_Inverse_Transition_classification_w_attention_results_BiGRU_{}_directions_{}_layers_{}_lr_{}_units_{}_epochs".format(config["optimizer_decay"], config["num_directions"], config["num_layers"], config["learning_rate"], config["hidden_size"], config["num_epochs"])
    else:
        config["result_filename"] = "results/paper_final/W_Decay_{}_Inverse_Transition_classification_results_BiGRU_{}_directions_{}_layers_{}_lr_{}_units_{}_epochs".format(config["optimizer_decay"], config["num_directions"], config["num_layers"], config["learning_rate"], config["hidden_size"], config["num_epochs"])
    for cv_idx, cv_fold in enumerate(rand_uid):
        ...
        loss = model.train_test_gru(train_act_inp, train_act_out, val_act_inp, val_act_out, val_trans_out)
        costs.append(loss)
        config["CV_index"] = cv_idx
        config["test_loss"] = loss
        config["time_elapsed"] = time.time()-start_time
        with open("Paper_tune_results.csv", 'a') as f:    
