...
class Custom_GRU(nn.Module):
    def __init__(self, config):
        ...
        if(self.attention_setting == True):
            ...
            self.attention_weights = new_parameter(1, self.hidden_size)
        else:
...
for parameter_combo in all_parameter_combinations:
    ...
    if(config["attention"] == True):
        config["result_filename"] = "results/summed_attention_unoptimized/Inverse_Transition_classification_w_attention_results_BiGRU_{}_directions_{}_layers_{}_lr_{}_units_{}_epochs".format(config["num_directions"], config["num_layers"], config["learning_rate"], config["hidden_size"], config["num_epochs"])
    else:
        config["result_filename"] = "results/summed_attention_unoptimized/Inverse_Transition_classification_results_BiGRU_{}_directions_{}_layers_{}_lr_{}_units_{}_epochs".format(config["num_directions"], config["num_layers"], config["learning_rate"], config["hidden_size"], config["num_epochs"])
