...
#SBATCH --job-name=GRU_inverse_Attention
#SBATCH --output=GRU_inverse_Attention.log
#SBATCH --mem=32G
#SBATCH -n 8
...
class Custom_GRU(nn.Module):
    def __init__(self, config):
        ...
        self.weights = new_parameter(self.hidden_size*self.num_directions, self.hidden_size*self.num_directions)
        self.attention = new_parameter(self.hidden_size*self.num_directions, 1)
        self.act = nn.Softmax()
        #self.act = nn.Tanh()
    def forward(self, x):
    ...
    def train_gru(self, train_inp, train_out, test_inp, test_act_out, test_out):
        ...
        with open(pred_fname, 'a') as f:
            predictions=np.vstack((list(activity_label_names)*3,np.hstack((test_act_out, torch.round(pred_t).cpu().detach().numpy(), pred_t.cpu().detach().numpy()))))
        ...
        with open(pred_fname, 'a') as f:
            predictions=np.vstack((list(transition_label_names)*3,np.hstack((test_out, torch.round(pred_t).cpu().detach().numpy(), pred_t.cpu().detach().numpy()))))
...
for parameter_combo in all_parameter_combinations:
    ...
    config["result_filename"] = "results/attention/Inverse_Transition_classification_results_BiGRU_{}_directions_{}_layers_{}_lr_{}_units_{}_epochs".format(config["num_directions"], config["num_layers"], config["learning_rate"], config["hidden_size"], config["num_epochs"])
    for cv_idx, cv_fold in enumerate(rand_uid):
        ...
        with open("Inverse_GRU_W_attention_tune_results.csv", 'a') as f:    
            #print(config)
